...
export function Terminal({
  ...
  outputs = {
    ...
    1: ["bun install v1.3.12", "+ @tanstack/react-router", "+ vite", "installed 42 packages in 180ms"],
    2: [
  ...
  const prompt = (
    <span className="text-neutral-500 font-mono">
      <span className="text-sky-400 font-semibold">{username}</span>
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "mx-auto w-full max-w-2xl px-4 font-mono text-xs sm:text-sm",
        className,
      )}
    >
      <div className="overflow-hidden rounded-xl border border-neutral-800/90 bg-neutral-950/95 shadow-[0_0_50px_rgba(0,0,0,0.8)] backdrop-blur-md">
        {/* Title Bar */}
        <div className="flex items-center gap-2 border-b border-neutral-800/80 bg-neutral-900/90 px-4 py-3">
          <div className="flex items-center gap-2">
            <div className="h-3 w-3 rounded-full bg-red-500/90 transition-colors hover:bg-red-600" />
            <div className="h-3 w-3 rounded-full bg-yellow-500/90 transition-colors hover:bg-yellow-600" />
            <div className="h-3 w-3 rounded-full bg-green-500/90 transition-colors hover:bg-green-600" />
          </div>
          <div className="flex-1 text-center">
            <span className="truncate text-xs font-mono text-neutral-400 tracking-wide">
              {username} — bash
            </span>
          </div>
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-80 sm:h-96 overflow-y-auto p-5 font-mono space-y-1.5"
        >
          {lines.map((line, i) => (
          ...
          {phase === "typing" && (
            <div className="leading-relaxed whitespace-pre-wrap">
              ...
              <span className="ml-0.5 inline-block h-4 w-2 bg-neutral-300 align-middle animate-pulse" />
            </div>
          )}
